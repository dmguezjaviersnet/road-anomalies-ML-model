...
def select_model(series_outls: pd.DataFrame, class_vector: list[int]):
    ...
    series_outls_scaled = pd.DataFrame(scaler.fit_transform(
        series_outls), columns=series_outls.columns)
...
def train_gs_cv(clsf, param_grid, X_train: pd.DataFrame, y_train: list[int]):
    ...
    hyp_estm_cv = GridSearchCV(estimator=clsf, param_grid=param_grid,
                               scoring=f1_scorer, cv=cross_validator, n_jobs=-1)
